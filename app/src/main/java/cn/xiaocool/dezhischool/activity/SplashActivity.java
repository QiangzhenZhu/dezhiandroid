...
//        StatService.onPause(this);
    ...
    private void LoginToWxt() {
            if (isLoginFromOther){
                startActivity(new Intent(SplashActivity.this, LoginActivity.class));
                finish();
            }else if (SPUtils.contains(mContext, LocalConstant.USER_ID)) {// 曾经登录过
    ...
    public void checkLoginFromOther(){
        Uri uri = Uri.parse(NetConstantUrl.CHECK_LOGIN_FROM_OTHER)
                .buildUpon()
                .appendQueryParameter("userid", (String) SPUtils.get(mContext,LocalConstant.USER_ID,""))
                .appendQueryParameter("registrationID", (String) SPUtils.get(mContext,LocalConstant.JPUSH_REGISTERID,""))
                .build();
        OkhttpUtil.sendOkHttpRequest(uri.toString(), new Callback() {
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                if (OkhttpUtil.isSuccess(data)){
                    ProgressUtil.dissmisLoadingDialog();
                    isLoginFromOther = true;
                    LoginToWxt();
                } else {
                    ProgressUtil.dissmisLoadingDialog();
                    isLoginFromOther = false;
                    LoginToWxt();
                }
            }
        });
